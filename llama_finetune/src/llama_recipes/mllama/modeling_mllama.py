...
from .configuration_mllama import diffag_config as Diffag_Config

from .diffag.diffag_xattn_manager import (
    ResNetBasedProjection, 
    DiffagFovProjection, 
    AttentionStore, 
    extend_mllama_xattn, 
    extend_llama_attention_with_xattn, 
    compute_cross_attention_flow,
    aggregate_attention,
    compute_perword_attention_dict,
)
from .diffag.diffag_utils import *
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    def __init__(self, config: MllamaConfig, diffag_args = None):
        super().__init__(config)

        # Convert diffag_args from dict to Diffag_Config if needed
        diffag_args = (lambda x: Diffag_Config(**x) if isinstance(x, dict) else x)(diffag_args)

        self.diffag_args = diffag_args
        self.global_step = 0
        self.max_steps = 1
        if self.diffag_args and self.diffag_args.diff_xattn_guidence:
            self.attention_controller_f = AttentionStore(diffag_args=self.diffag_args)
        else:
            self.attention_controller_f = None
        ...
        if diffag_args is not None and diffag_args.diff_xattn_guidence:
            # Flag to indicate if extensions have been applied
            self._extensions_applied = False
            # Your custom initialization here, d_in is head size
            if self.diffag_args.diffag_maxmax:
                d_in = 1
            elif self.diffag_args.diffag_xattn_flow:
                d_in = 32
            else:
                d_in = 32
            d_h = self.diffag_args.diffag_hidden_dim
            if diffag_args.diff_guidance_net == 'fov':
                self.diffag_projection = DiffagFovProjection(self.diffag_args, d_in=d_in, d_h=d_h)
            elif diffag_args.diff_guidance_net == 'resnet50pretrain':
                # Initialize ResNet projection layer
                self.diffag_projection = ResNetBasedProjection(in_channels=d_in, pretrained=False)
            elif diffag_args.diff_guidance_net == 'resnet50scratch':
                # Initialize ResNet projection layer
                self.diffag_projection = ResNetBasedProjection(in_channels=d_in, pretrained=False)
            else:
                raise ValueError("Unknown diff_guidance_net")

            self.apply_xattn_extensions()
    ...
    def apply_xattn_extensions(self):
        if self.diffag_args and self.diffag_args.diff_xattn_guidence and not self._extensions_applied:
            # Ensure synchronization across GPUs before extending
            if torch.distributed.is_initialized():
                torch.distributed.barrier()

            self.attention_controller_f = AttentionStore(diffag_args=self.diffag_args)
            # extend_llama_attention_with_xattn(self, self.attention_controller_f, self.diffag_args)
            extend_mllama_xattn(self, self.attention_controller_f, self.diffag_args)

            # Synchronize after modifications
            if torch.distributed.is_initialized():
                torch.distributed.barrier()

            # Ensure model is moved back to the correct device
            self.to(self.device)
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            if self.attention_controller_f is not None:
                self.attention_controller_f.num_tiles = num_tiles
        ...
        if cross_attention_mask is not None:
            if self.attention_controller_f is not None:
                self.attention_controller_f.cross_attention_mask_raw = cross_attention_mask
        ...
        # Optionally compute_diffag_loss
        mllama_cross_attn = True

        if steps is not None:
            self.global_step, self.max_steps, self.rank = steps

        if torch.distributed.is_initialized():
            torch.distributed.barrier()  # Synchronize all GPUs before compute_diffag_loss
        
        if self.diffag_args is not None and self.diffag_args.diff_xattn_guidence and not eval_mode:
            # Extract text_mask and vision_mask
            if not mllama_cross_attn:
                text_mask, vision_mask = extract_masks(inputs, outputs.logits)
            
            # Initialize list for decoded text
            decoded_text = []

            # Extract the text tokens using text_mask[b] for outputs.logits[b]
            bs = outputs.logits.shape[0]
            for b in range(bs):
                # Apply text_mask[b] to extract only the text tokens
                if not mllama_cross_attn:
                    text_logits = outputs.logits[b][text_mask[b]]
                else:
                    text_logits = outputs.logits[b]
                
                # Decode the text
                token_ids = text_logits.argmax(dim=-1)
                decoded_text.append(self.processor.tokenizer.batch_decode(token_ids, skip_special_tokens=True))

            # compute xattn saliency
            # first compute mean layer_attention_store l b h (r1 r2) n1
            layer_attention_store = aggregate_attention(
                self.attention_controller_f,
                average_att_ch=True,
                use_layer=self.diffag_args.use_layer,
                num_last_layers=self.diffag_args.num_last_layers,
                attn_select_layer=self.diffag_args.attn_select_layer,
            )

            l, b, h, r1r2, n1 = layer_attention_store.shape
            res = int(np.ceil(np.sqrt(r1r2)))  # Get the smallest integer r such that r*r >= n2

            # compute aggregated attn_c over layers
            if self.diffag_args.diffag_maxmax:
                # Compute max over heads (dim=2) and then max over layers (dim=0)
                # This finds the maximum attention across all heads and layers for each text-patch pair.
                aggregated_attention_map = torch.max(torch.max(layer_attention_store, dim=2).values, dim=0).values  # Shape (B, r1r2, n1)
                attn_c = rearrange(aggregated_attention_map, "b (r1 r2) n1 -> (b n1) r1 r2", r1=res, r2=res)
                attn_c = attn_c.unsqueeze(1) # (b n1) h r1 r2
                h = 1
            elif self.diffag_args.diffag_xattn_flow:
                aggregated_attention_map = compute_cross_attention_flow(
                    layer_attention_store, 
                    sum_propogate=self.diffag_args.diffag_sum_propogate,
                    diffag_reg_propogate=self.diffag_args.diffag_reg_propogate,
                    ) # b h (r1 r2) n1
                attn_c = rearrange(aggregated_attention_map, "b h (r1 r2) n1 -> (b n1) h r1 r2", r1=res, r2=res)
                # attn_c = aggregated_attention_map_sfotmax.unsqueeze(1) # (b n1) h r1 r2
                # h = 1
            else:
                if self.diffag_args.diffag_mean_layer:
                    if self.diffag_args.layer_extract == "average":
                        layer_attention_store = layer_attention_store.mean(dim=0) # b h (r1 r2) n1
                    elif self.diffag_args.layer_extract == "max":
                        layer_attention_store, _ = layer_attention_store.max(dim=0) # b h (r1 r2) n1
                    attn_c = rearrange(layer_attention_store, "b h (r1 r2) n1 -> (b n1) h r1 r2", r1=res, r2=res)
                else:
                    attn_c = rearrange(layer_attention_store, "l b h (r1 r2) n1 -> (l b n1) h r1 r2", r1=res, r2=res)

            # optional go through conv projection and get attention maps
            if self.diffag_args.diffag_skip_proj and (self.diffag_args.diffag_xattn_flow or self.diffag_args.diffag_maxmax):
                attention_maps = rearrange(attn_c, "(b n1) h r1 r2 -> b h (r1 r2) n1", n1=n1) # b h (r1 r2) n1
            else:
                # if is_deepspeed_zero3_enabled():
                #     print('DEBUG is_deepspeed_zero3_enabled')
                #     with deepspeed.zero.GatheredParameters(self.diffag_projection.parameters(), modifier_rank=0):
                #         attn_proj = self.diffag_projection(attn_c) 
                #         if self.diffag_args.diffag_mean_layer or self.diffag_args.diffag_xattn_flow or self.diffag_args.diffag_maxmax:
                #             attn_mlp = rearrange(attn_proj, "(b n1) h r1 r2 -> b h (r1 r2) n1", b=b, n1=n1)
                #         else:
                #             attn_mlp = rearrange(attn_proj, "(l b n1) h r1 r2 -> l b h (r1 r2) n1", l=l, b=b, n1=n1)
                #             if self.diffag_args.layer_extract == "average":
                #                 attn_mlp = attn_mlp.mean(dim=0) # b h (r1 r2) n1
                #             elif self.diffag_args.layer_extract == "max":
                #                 attn_mlp, _ = attn_mlp.max(dim=0) # b h (r1 r2) n1
                #         attention_maps = self.diffag_projection._normalize_attn(attn_mlp) # b h (r1 r2) n1
                # else:
                #     print('DEBUG NOT ! is_deepspeed_zero3_enabled')
                attn_proj = self.diffag_projection(attn_c) # (b n1) 1 r1 r2 or (l b n1) h r1 r2
                if self.diffag_args.diffag_mean_layer or self.diffag_args.diffag_xattn_flow or self.diffag_args.diffag_maxmax:
                    attn_mlp = rearrange(attn_proj, "(b n1) h r1 r2 -> b h (r1 r2) n1", b=b, n1=n1)
                else:
                    attn_mlp = rearrange(attn_proj, "(l b n1) h r1 r2 -> l b h (r1 r2) n1", l=l, b=b, n1=n1)
            ...
            # Load SD attentions
            sd_attn = deepcopy(sd_attn)
            attn_dict_list = []
            
            # Collect per-word attentions
            for b in range(bs):
                attn_dict = compute_perword_attention_dict(
                    attention_maps[b][0],
                    decoded_text[b],
                )
                attn_dict_list.append(attn_dict)

            if self.diffag_args.diffag_plot_before_proj:
                attn_c_plot_list = []

                if self.diffag_args.diffag_mean_layer or self.diffag_args.diffag_xattn_flow or self.diffag_args.diffag_maxmax:
                    attn_c_maps = rearrange(attn_c, "(b n1) h r1 r2 -> b h (r1 r2) n1", n1=n1) # b h (r1 r2) n1
                else:
                    if self.diffag_args.layer_extract == "average":
                        attn_c_maps = layer_attention_store.mean(dim=0) # b h (r1 r2) n1
                    elif self.diffag_args.layer_extract == "max":
                        attn_c_maps, _ = layer_attention_store.max(dim=0) # b h (r1 r2) n1
                ...
                for b in range(bs):
                    ...
                    attn_c_plot_list.append(attn_dict)

            # Compute attention loss
            total_loss_mse = torch.tensor(0.0, device=self.device, requires_grad=True)
            len_loss_mse = 0

            if self.global_step % self.diffag_args.plot_step == 0:
                attn_keys_save = [[] for _ in range(bs)]
            for b in range(bs):
                attn_dict = attn_dict_list[b]
                # print(f'DEBUG: attn_dict_list[{b}] keys: {attn_dict.keys()}')
                # print(f'DEBUG: decoded_text[{b}]: {decoded_text[b]}')
                if self.diffag_args.sub_obj:
                    if self.diffag_args.sub_obj_exclude_reject:
                        list_conversations = extract_before_rejected_answer(inputs["conversations"][b])
                    else:
                        list_conversations = inputs["conversations"][b]
                    subject_object_list = find_subject_and_object_in_sentences(list_conversations)
                    # Convert to sets and find the intersection (overlapping words)
                    overlapping_words = set(subject_object_list) & set(attn_dict.keys()) & set(sd_attn[b].keys())
                    # Convert back to list if needed
                    attn_dict_keys = list(overlapping_words)
                else:
                    attn_dict_keys = attn_dict.keys()
                for attn_key in attn_dict_keys:
                    if (self.diffag_args.noun_only and not is_noun(attn_key)):
                        continue
                    if (self.diffag_args.adj_only and not is_adj(attn_key)):
                        continue
                    if (self.diffag_args.is_nav and not is_nav(attn_key)):
                        continue
                    if (
                        attn_key in sd_attn[b].keys() 
                        and attn_dict[attn_key].mean() > 0
                        and not (attn_key == '' or is_punctuation(attn_key))
                    ):
                        sd_key = attn_key
                        loss_mse = nn.MSELoss()(sd_attn[b][sd_key], attn_dict[attn_key])
                        if not torch.isnan(loss_mse):
                            # print(f"computed MSE loss={loss_mse} between attn_key={attn_key} and sd_key={sd_key}")
                            total_loss_mse = total_loss_mse + loss_mse
                            len_loss_mse += 1
                        # if b == 0 and self.global_step % self.diffag_args.plot_step == 0 and is_noun(sd_key):
                        if self.global_step % self.diffag_args.plot_step == 0 and is_nav(attn_key) and len(attn_keys_save[b]) < 3:
                            attn_keys_save[b].append(attn_key)

            if self.global_step % self.diffag_args.plot_step == 0:        
                # save the combined images with rgb_img
                if self.diffag_args.diffag_plot_before_proj:
                    combined_img = save_combined_images(rgb_img, bs, sd_attn, attn_dict_list, attn_keys_save, self.global_step, output_dir, attn_c_plot_list=attn_c_plot_list)
                else:
                    combined_img = save_combined_images(rgb_img, bs, sd_attn, attn_dict_list, attn_keys_save, self.global_step, output_dir)
            else:
                combined_img = None

            # if self.diffag_args.len_reg_mse:
            average_loss_attn = (total_loss_mse + 1e-8) / (len_loss_mse + 1e-8)
            # else:
            #     if len_loss_mse > 0:
            #         average_loss_attn = total_loss_mse / len_loss_mse
            #     else:
            #         average_loss_attn = torch.tensor(
            #             0.0, device=self.device, requires_grad=True
            #         )

            # Apply MSE decay and add to final loss
            if self.diffag_args.mse_decay:
                mse_weight = exponential_decay_scheduler(
                    self.diffag_args.sd_xattn_loss_scale,
                    self.global_step,
                    self.max_steps,
                    factor=self.diffag_args.mse_decay_factor,
                    inverse=self.diffag_args.inverse_mse_decay,
                )
            else:
                mse_weight = self.diffag_args.sd_xattn_loss_scale

            average_loss_attn = average_loss_attn * mse_weight
            if (
                self.diffag_args is not None and self.diffag_args.diff_xattn_guidence and 
                self.diffag_args.diffag_pretrain_diffag_net != 0 
                and self.global_step < self.diffag_args.diffag_pretrain_diffag_net
            ):
                epsilon = 1e-7
                outputs.loss = outputs.loss * epsilon # ignore caption loss during pretrain stage
            
            if (
                self.diffag_args is not None and self.diffag_args.diff_xattn_guidence 
                and self.global_step >= self.diffag_args.diffag_pretrain_diffag_net
                and self.diffag_args.loss_linear_alpha
            ):
                current_step = max(0, self.global_step - self.diffag_args.diffag_pretrain_diffag_net)
                total_step = max(1, self.max_steps - self.diffag_args.diffag_pretrain_diffag_net)
                loss_alpha = current_step / total_step + 1e-7
                outputs.loss = outputs.loss * loss_alpha
            
            outputs.loss = outputs.loss + average_loss_attn
            # print(f"Scaled SD attn MSE Loss: {average_loss_attn}, captioning loss: {outputs.loss.item()-average_loss_attn.item()}, final loss: {outputs.loss}")
        
            # if self.diffag_args is not None and self.diffag_args.diff_xattn_guidence:
            #     wandb.log({"average_loss_attn": average_loss_attn.item()})
            # Log the custom loss
            if wandb_run and self.rank == 0:
                logs = {
                    "diffag/text_regression_loss": (outputs.loss.item()-average_loss_attn.item()),
                    "diffag/x-attention_mse_loss": average_loss_attn.item(),
                    "diffag/total_loss": outputs.loss.item(),
                    "diffag/len_loss_mse": len_loss_mse,
                    "diffag/gpu_step": self.global_step,
                }

                # Check if the combined image exists
                if combined_img is not None:
                    # Log the image to wandb
                    logs["diffag_vis/combined_img"] = wandb.Image(combined_img, caption=f"Combined Image at gpu_step {self.global_step}")

                # Log the metrics and image (if available), we set commit to false keeping the same step
                wandb_run.log(logs, commit=False)

                # Now remove the non-serializable image object from logs
                if "combined_img" in logs:
                    del logs["combined_img"]

            if self.attention_controller_f is not None:
                self.attention_controller_f.reset()
                
        if torch.distributed.is_initialized():
            torch.distributed.barrier()  # Synchronize all GPUs before compute_diffag_loss

        if self.diffag_args.diff_xattn_guidence and not eval_mode:
            return outputs, average_loss_attn.item()
        else:
            return outputs
