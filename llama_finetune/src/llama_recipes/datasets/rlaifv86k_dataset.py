...
def get_custom_dataset(dataset_config, processor, split="train", split_ratio=0.9):
    raw_data = json.load(open(dataset_config.data_path, "r"))
    ...
    for idx, sample in enumerate(data):
        if dataset_config.image_train_attn_path:
            image_path = sample["image"]  # Get the image path from the image object
        ...
        for turn in dialog:
            ...
            if role == "user":
                if len(formatted_dialog) == 0:
                    formatted_dialog.append({
                        "role": "user",
                        "content": [{"type": "image"}, {"type": "text", "text": content.strip()}]
                    })
                else:
                    formatted_dialog.append({
                        "role": "user",
...
class CustomDataCollator:
    ...
    def __call__(self, samples):
        ...
        # Load `sd_attn` only when required
        ...
        if self.dataset_config.image_train_attn_path:
            batch["sd_attn"] = sd_attn_list if sd_attn_list else None
...
def get_data_collator(processor,dataset_config):
    return CustomDataCollator(processor,dataset_config)