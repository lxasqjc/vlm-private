...
def save_model_and_optimizer_sharded(model, rank, cfg,optim=None, epoch=0, iter_step=0):
    """save model and optimizer via sharded_state_dict to save_dir"""
    
    folder_name = (
        cfg.dist_checkpoint_root_folder
        + "/"
        + cfg.output_dir.split('/')[-1] + f'_e{epoch}_it{iter_step}'
    )
...
def save_fsdp_model_checkpoint_full(
    ...
    if rank == 0:
        ...
        save_name = cfg.model_name.replace("/", "--") + ".pt"
        ...
        print(f"model checkpoint saved for epoch {epoch}" + (f" iter_step {iter_step}" if iter_step != -1 else "") + f" at {save_full_path}\n")
...
def save_optimizer_checkpoint(model, optimizer, rank, cfg, epoch=1, iter_step=-1):
...
def save_peft_checkpoint(model, model_path, epoch=0, iter_step=0, include_ep_it=True):
    """save_pretrained peft model"""

    model_path = model_path + f'_e{epoch}_it{iter_step}'
...
def save_model_checkpoint(model, output_dir, epoch=0, iter_step=0):
    """save model when not peft and on single device"""
    
    output_file = Path(f"{output_dir}_e{epoch}_it{iter_step}") / "model.pt"
