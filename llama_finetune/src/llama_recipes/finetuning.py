...
from llama_recipes.configs import (
    ...
    diffag_config as DIFFAG_CONFIG,
)
...
from llama_recipes.utils.train_utils import (
    clear_gpu_cache,
    freeze_llm_transformer_layers,
    freeze_vis_transformer_layers,
    get_policies,
    print_model_size,
    setup,
    setup_environ_flags,
...
from llama_recipes.diffag.diffag_xattn_manager import *
...
def setup_wandb(train_config, fsdp_config, **kwargs):
    ...
    wandb_config.name = train_config.output_dir.split('/')[-1]
    update_config(wandb_config, **kwargs)
    init_dict = dataclasses.asdict(wandb_config)
    if train_config.multi_stage_train:
        init_dict['resume'] = 'allow'
...
def main(**kwargs):
    ...
    diffag_args = DIFFAG_CONFIG()
    train_config.model_name = os.path.expanduser(train_config.model_name)

    update_config((train_config, fsdp_config, diffag_args), **kwargs)

    if len(diffag_args.diff_attend_layers) > 0:
        diffag_args.diff_attend_layers = list(diffag_args.diff_attend_layers)
    else:
        diffag_args.diff_attend_layers = []
    ...
    config = AutoConfig.from_pretrained(os.path.expanduser(train_config.model_name))
    if config.model_type == "mllama":
        is_vision = True
        model = MllamaForConditionalGeneration.from_pretrained(
            os.path.expanduser(train_config.model_name),
            ...
            diffag_args=asdict(diffag_args),  # Pass the additional arguments
        )
    ...
    # Load the pre-trained peft model checkpoint and continue with fsdp
    # this strategy is not working on scp
    if train_config.from_peft_checkpoint and not train_config.use_peft:
        print(f'peft checkpoint provided, load from{train_config.from_peft_checkpoint}')
    ...
    if train_config.use_peft:
        ...
        # Enable specific layers based on the 'diff_guidance_net' argument
        if diffag_args.diff_xattn_guidence:
            model.diffag_projection.requires_grad_(True)
            if diffag_args.diffag_learn_xattn:
                for name, param in model.named_parameters():
                    if 'diffxattn' in name:
    ...
    if train_config.enable_fsdp:
        ...
        if not train_config.use_peft and train_config.freeze_layers:
            freeze_llm_transformer_layers(model.language_model, train_config, diffag_args.diff_attend_layers)
            freeze_vis_transformer_layers(model.vision_model, train_config)
    ...
    if diffag_args.image_train_attn_path != "":
        dataset_config.image_train_attn_path = diffag_args.image_train_attn_path
